...
export const SearchCommand = new Command("search").description(
  "Full-text search using Nouveau (CouchDB 3.x Lucene-based search)",
);
...
SearchCommand.command("query <ddoc> <index> <query> [db]")
  ...
  .option(
    "--fields <fields>",
    "Comma-separated fields to return from the index",
  )
  ...
  .option(
    "--highlight-post <tag>",
    "HTML tag to close highlight (default </em>)",
  )
  ...
  .action(
    async (
      ...
      try {
        ...
        if (options.limit)
          searchOptions.limit = Number.parseInt(options.limit as string, 10);
        if (options.bookmark)
          searchOptions.bookmark = options.bookmark as string;
        if (options.includeDocs) searchOptions.include_docs = true;
        if (options.fields) {
          searchOptions.fields = (options.fields as string)
            .split(",")
            .map((f: string) => f.trim());
        }
        if (options.sort) {
        ...
        if (options.counts) {
          searchOptions.counts = (options.counts as string)
            .split(",")
            .map((f: string) => f.trim());
        }
        ...
        if (options.highlightPre)
          searchOptions.highlight_pre_tag = options.highlightPre as string;
        if (options.highlightPost)
          searchOptions.highlight_post_tag = options.highlightPost as string;

        const result = await client.nouveauSearch(
          name,
          ddocName,
          index,
          query,
        ...
        if (
          ...
          msg.includes("nouveau") ||
          msg.includes("search")
        ) {
          console.error(pc.red(`Error: ${msg}`));
          console.error(
            pc.dim(
              "  Nouveau search requires CouchDB 3.2+ with search enabled.",
            ),
          );
          console.error(
            pc.dim("  Check if your CouchDB instance has Nouveau configured."),
          );
        } else {
          console.error(pc.red(`Error: ${msg}`));
        }
