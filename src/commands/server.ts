...
export const ServerCommand = new Command("server").description(
  "Server information and cluster management",
);
...
ServerCommand.command("info")
  ...
  .action(async (options) => {
    try {
      ...
      if (info.vendor) {
        console.log(
          `  ${pc.dim("vendor:")}  ${info.vendor.name}${info.vendor.version ? ` ${info.vendor.version}` : ""}`,
        );
      }
    } catch (error) {
      console.error(
...
ServerCommand.command("membership")
...
ServerCommand.command("tasks")
  ...
  .action(async (options) => {
    try {
      const client = await getClient();
      let tasks = (await client.getActiveTasks()) as Array<
      ...
      for (const task of tasks) {
        ...
        if (task["database"])
          console.log(`    ${pc.dim("database:")}  ${task["database"]}`);
        if (task["design_document"])
          console.log(`    ${pc.dim("ddoc:")}      ${task["design_document"]}`);
        ...
        if (task["started_on"]) {
          const ts = new Date(
            Number(task["started_on"]) * 1000,
          ).toLocaleTimeString();
          console.log(`    ${pc.dim("started:")}   ${ts}`);
        }
        // Replication-specific
        if (task["source"])
          console.log(`    ${pc.dim("source:")}    ${task["source"]}`);
        if (task["target"])
          console.log(`    ${pc.dim("target:")}    ${task["target"]}`);
...
ServerCommand.command("scheduler")
      ...
      if (options.docs) {
        ...
        for (const doc of result.docs) {
          ...
          if (doc.error_count > 0)
            console.log(
              `    ${pc.dim("errors:")}   ${pc.red(String(doc.error_count))}`,
            );
          if (doc.last_updated)
        ...
        console.log(
          pc.cyan(
            `⚙️  Scheduler Jobs (${result.total_rows} total, ${result.jobs.length} shown)`,
          ),
        );
        ...
        for (const job of result.jobs) {
          ...
          if (job.start_time)
            console.log(`    ${pc.dim("started:")} ${job.start_time}`);

          const lastEvent = job.history?.[0];
          if (lastEvent) {
            console.log(
              `    ${pc.dim("last:")}    ${lastEvent.type}${lastEvent.reason ? " – " + lastEvent.reason : ""}`,
            );
          }
        }
      }
...
ServerCommand.command("stats")
  ...
  .action(async (options) => {
    try {
      ...
      const httpd = (stats as Record<string, unknown>)["httpd"] as
        | Record<string, unknown>
        | undefined;
      const couchdb = (stats as Record<string, unknown>)["couchdb"] as
        | Record<string, unknown>
        | undefined;

      if (httpd) {
        const requests = httpd["requests"] as
          | Record<string, unknown>
          | undefined;
        const bulkRequests = httpd["bulk_requests"] as
          | Record<string, unknown>
          | undefined;
        if (requests)
          console.log(
            `  ${pc.dim("httpd.requests:")}      ${(requests["value"] as number) ?? 0}`,
          );
        if (bulkRequests)
          console.log(
            `  ${pc.dim("httpd.bulk_requests:")} ${(bulkRequests["value"] as number) ?? 0}`,
          );
      }

      if (couchdb) {
        const openDbs = couchdb["open_databases"] as
          | Record<string, unknown>
          | undefined;
        const openFiles = couchdb["open_os_files"] as
          | Record<string, unknown>
          | undefined;
        if (openDbs)
          console.log(
            `  ${pc.dim("couchdb.open_dbs:")}    ${(openDbs["value"] as number) ?? 0}`,
          );
        if (openFiles)
          console.log(
            `  ${pc.dim("couchdb.open_files:")}  ${(openFiles["value"] as number) ?? 0}`,
          );
      }
